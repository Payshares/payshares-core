...
This document describes various aspects of running `payshares-core` for **system administrators** (but may be useful to a broader audience).

## Introduction

Payshares Core is responsible for communicating directly with and maintaining 
the Payshares peer-to-peer network. For a high-level introduction to Payshares Core, [watch this talk](https://www.youtube.com/watch?v=pt_mm8S9_WU) on the architecture and ledger basics:

[![Introduction to Payshares Core](https://i.ytimg.com/vi/pt_mm8S9_WU/hqdefault.jpg "Introduction to Payshares Core")](https://www.youtube.com/watch?v=pt_mm8S9_WU)

It will also be useful to understand how [data flows](https://www.payshares.org/developers/payshares-core/software/core-data-flow.pdf) and is stored in payshares-core.

## Why run a node?

Run payshares-core if you want to:
* Obtain the most up-to-date and reliable data from the Payshares network
* Generate extended meta data on activity within the Payshares network (change tracking, etc)
* Submit transactions and their confirmations without depending on a third party
* Extended control on which parties to trust in the Payshares network
* Participate in validating the Payshares network


## Building
See [readme](https://github.com/payshares/payshares-core/blob/master/README.md) for build instructions.
We also provide a [docker container](https://github.com/payshares/docker-payshares-core-horizon) for a potentially quicker set up than building from source.

## Package based Installation
If you are using Ubuntu 16.04 LTS we provide the latest stable releases of [payshares-core](https://github.com/payshares/payshares-core) and [payshares-horizon](https://github.com/payshares/go/tree/master/services/horizon) in Debian binary package format.

See [detailed installation instructions](https://github.com/payshares/packages#sdf---packages)

## Configuring
All configuration for payshares-core is done with a TOML file. By default 
payshares-core loads 

`./payshares-core.cfg`

, but you can specify a different file to load on the command line:

`$ payshares-core --conf betterfile.cfg` 

The [example config](https://github.com/payshares/payshares-core/blob/master/docs/payshares-core_example.cfg) describes all the possible 
configuration options.

Here is an [example test network config](https://github.com/payshares/docker-payshares-core-horizon/blob/master/testnet/core/etc/payshares-core.cfg) for connecting to the test network.

Here is an [example public network config](https://github.com/payshares/docs/blob/master/other/payshares-core-validator-example.cfg) for connecting to the public network.

The examples in this file don't specify `--conf betterfile.cfg` for brevity.

## Running
Payshares-core can be run directly from the command line, or through a supervision 
system such as `init`, `upstart`, or `systemd`.

Payshares-core sends logs to standard output and `payshares-core.log` by default, 
...
`$ payshares-core -c "ll?level=debug"`
...
`$ payshares-core -c "ll?level=debug&partition=history"` 
...
Payshares-core can be gracefully exited at any time by delivering `SIGINT` or
...
Payshares-core can also be packaged in a container system such as Docker, so long 
as `BUCKET_DIR_PATH`, `TMP_DIR_PATH`, and the database are stored on persistent 
volumes. For an example, see [docker-payshares-core](https://github.com/payshares/docker-payshares-core-horizon).

Note: `BUCKET_DIR_PATH` and `TMP_DIR_PATH` *must* reside on the same volume
as payshares-core needs to rename files between the two.

## Administrative commands
While running, interaction with payshares-core is done via an administrative 
HTTP endpoint. Commands can be submitted using command-line HTTP tools such 
as `curl`, or by 

`$ payshares-core -c <command>`
...
See a [list of other validators](https://github.com/payshares/docs/blob/master/validators.md).

## Database
Payshares-core stores the state of the ledger in a SQL database. This DB should 
...
see [example config](https://github.com/payshares/payshares-core/blob/master/docs/payshares-core_example.cfg).

When running payshares-core for the first time, you must initialize the database:

`$ payshares-core --newdb`
...
## Buckets
Payshares-core stores a duplicate copy of the ledger in the form of flat XDR files 
...
## History archives
Payshares-core normally interacts with one or more "history archives," which are 
...
The [example config](https://github.com/payshares/payshares-core/blob/master/docs/payshares-core_example.cfg) 
shows how to configure a history archive through command templates. 

While it is possible to run a payshares-core node with no configured history 
...
`$ payshares-core --newhist <historyarchive>`
 ...
 * the base reserve used to calculate the stakk balance needed to store things in the ledger
...
## A brief explanation
An important distinction in Payshares compared to traditional quorum based
...
1. `$ payshares-core -c 'upgrades?mode=set&upgradetime=2018-01-31T20:00:00Z&protocolversion=9'`

2. `$ payshares-core -c info`
...
`$ payshares-core --c 'quorum?node=$sdf1` or `$ payshares-core --c 'quorum?node=@GABCDE` 
...
* [architecture.md](https://github.com/payshares/payshares-core/blob/master/docs/architecture.md) 
