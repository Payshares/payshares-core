...
template <typename T> struct KeyFunctions
{
    ...
    toKeyType(psrKey::PsrKeyVersionByte keyVersion);
    static psrKey::PsrKeyVersionByte
...
toPsrKey(T const& key)
{
    return psrKey::toPsrKey(KeyFunctions<T>::toKeyVersion(key.type()),
...
{
    return toPsrKey(key).substr(0, 5);
}
...
toShortString(T const& key)
{
    return SecretValue{toPsrKey(key).value.substr(0, 5)};
}

std::size_t getKeyVersionSize(psrKey::PsrKeyVersionByte keyVersion);

template <typename T>
T
fromPsrKey(std::string const& s)
{
    T key;
    uint8_t verByte;
    std::vector<uint8_t> k;
    if (!psrKey::fromPsrKey(s, verByte, k))
    {
        throw std::invalid_argument("bad " + KeyFunctions<T>::getKeyTypeName());
    }

    psrKey::PsrKeyVersionByte ver =
        static_cast<psrKey::PsrKeyVersionByte>(verByte);
    if (!KeyFunctions<T>::getKeyVersionIsSupported(ver) ||
        (k.size() != getKeyVersionSize(ver)) ||
        (s.size() != psrKey::getPsrKeySize(getKeyVersionSize(ver))))
    {
        throw std::invalid_argument("bad " + KeyFunctions<T>::getKeyTypeName());
    }
